...
export function draw() {
    ...
    if (st.knockdown) {
        // v18: the arena and every enemy sink into the dark so the count, the ref
        // and the prompt are the only things to read.
        ctx.fillStyle = `rgba(0, 0, 0, ${(0.72 * Math.max(0.25, st.knockdown.fall)).toFixed(3)})`;
