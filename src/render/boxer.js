...
export function drawBoxer(ctx, entity, isPlayer, opacity = 1, isTrail = false) {
    ...
    // v19 LOADED CROSS — Mega Buster charge on the cross glove: a flicker while it
    // builds, then a pulsing, colour-strobing glow with orbiting sparks once loaded.
    if (isPlayer && !isTrail && entity.charging && st.progressionMods.loadedCross) {
        ...
        ctx.save();
        ctx.globalAlpha = opacity * (loaded ? 0.9 : 0.35 + 0.45 * k * (0.6 + 0.4 * Math.sin(t * 0.05)));
        ctx.fillStyle = flick; ctx.shadowColor = flick; ctx.shadowBlur = loaded ? 22 : 10;
        dc(rrX, rrY, gS + 2 + (loaded ? 4 + Math.sin(t * 0.03) * 2 : k * 3), true, false);
        ctx.shadowBlur = 0;
        const n = loaded ? 4 : 2;
        for (let i = 0; i < n; i++) {
            const a = t * (loaded ? 0.018 : 0.012) + i * Math.PI * 2 / n, r = gS + 9;
            ctx.globalAlpha = opacity; ctx.fillStyle = loaded ? '#ffffff' : col;
